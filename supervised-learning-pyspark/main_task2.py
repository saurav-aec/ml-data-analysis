...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: MobilePricePredictorSVM <trainFile> <testFile> ", 
        file=sys.stderr)
        exit(-1)
    ...
    sc = SparkContext(appName="MobilePricePredictorSVM")
    ...
    svmModel = SVMWithSGD.train(featureRdd, regParam=0.2)

    labelsAndPreds = testFeatureRdd.map(
        lambda p: (p.label, svmModel.predict(p.features)))
